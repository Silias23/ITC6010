...
def multinomial_nb_count_vectorizer(x_train, y_train, x_test, y_test, stop_words):
    ...
    model = make_pipeline(CountVectorizer(ngram_range=(1, 3), stop_words=stop_words), SVC())
    model.fit(x_train, y_train)
    preds = model.predict(x_test)

    print("Model: CountVectorizer + SVM w/o stopwords")
    ...
    ax.set_title('CountVectorizer + SVM w/o stopwords')
    plt.show()
...
def multinomial_nb_tfidf_vectorizer(x_train, y_train, x_test, y_test, stop_words):
    model = make_pipeline(TfidfVectorizer(ngram_range=(1, 3), stop_words=stop_words), SVC())

    model.fit(x_train, y_train)
    preds = model.predict(x_test)

    print("Model: TfidfVectorizer + SVC w/o stopwords")
    print(f"Number of features: {len(model[0].vocabulary_)}")
    print("Accuracy: {:.4f}\n".format(accuracy_score(y_test, preds)))
    mat = confusion_matrix(y_test, preds)
    ax = plt.axes()
    ...
    plt.xlabel('Predicted label')
    plt.ylabel('True label')
    ax.set_title('TfidfVectorizer + SVC w/o stopwords')
    plt.show()
    print(classification_report(y_test, preds, labels=model.classes_))
